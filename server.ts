...
function getNextApiKey() {
  ...
  for (const [key, value] of Object.entries(process.env)) {
    if (key.toUpperCase().startsWith('GEMINI_API_KEY') && value) {
...
async function startServer() {
  ...
  app.get('/api/admin/stats', (req, res) => {
    ...
    for (const [k, v] of Object.entries(process.env)) {
      if (k.toUpperCase().startsWith('GEMINI_API_KEY') && v) {
        keys.push(...v.split(','));
      }
      if (k.toUpperCase().includes('API_KEY') && v) {
        allEnvKeys.push({ name: k, masked: v.substring(0, 4) + '***' });
      }
    }
  ...
  app.post('/api/study-function', async (req, res) => {
    try {
      const { expression, mValue, pointX, language } = req.body;

      let systemInstruction = `أنت أستاذ جزائري خبير في الرياضيات (pro dali ai).
...
${mValue ? `(الرجاء إجراء المناقشة الوسيطية إن كان الوسيط m موجوداً في العبارة، علماً أن القيمة الحالية للرسم هي ${mValue})` : ''}`;
  ...
  app.post('/api/study-question', async (req, res) => {
    try {
      const { expression, question, language } = req.body;
